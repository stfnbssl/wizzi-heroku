...
class PluginProductionController {
    constructor() {
        ...
        this.initialize = (initValues) => {
            ...
            this.router.get("/new", makeHandlerAwareOfAsyncErrors(index_1.webSecured), makeHandlerAwareOfAsyncErrors(this.getNewPluginForm));
            this.router.post("/new", makeHandlerAwareOfAsyncErrors(index_1.webSecured), makeHandlerAwareOfAsyncErrors(this.postPlugin));
            this.router.get("/update/:id", makeHandlerAwareOfAsyncErrors(index_1.webSecured), makeHandlerAwareOfAsyncErrors(this.getUpdatePluginForm));
            this.router.post("/update", makeHandlerAwareOfAsyncErrors(index_1.webSecured), makeHandlerAwareOfAsyncErrors(this.putPlugin));
            this.router.get("/delete/:id", makeHandlerAwareOfAsyncErrors(index_1.webSecured), makeHandlerAwareOfAsyncErrors(this.getDeletePluginForm));
            this.router.post("/delete", makeHandlerAwareOfAsyncErrors(index_1.webSecured), makeHandlerAwareOfAsyncErrors(this.deletePlugin));
            this.router.get("/props", makeHandlerAwareOfAsyncErrors(index_1.webSecured), makeHandlerAwareOfAsyncErrors(this.getPluginProperties));
        };
        ...
        this.postPlugin = (request, response) => tslib_1.__awaiter(this, void 0, void 0, function* () {
            return (0, meta_1.getTemplatePackiFiles)(request.body.meta_id, request.body.meta_propsValues ? JSON.parse(request.body.meta_propsValues) : {}, request.query.context, request.body.context ? JSON.parse(request.body.context) : {}).then((packiFiles) => (0, plugin_1.createPluginProduction)(request.session.user.username, request.body.pp_name, request.body.pp_description, JSON.stringify((0, utils_2.mergePackiFiles)(packiFiles, getPackiFiles()))).then((result) => {
                if (result.ok) {
        ...
        (request, response) => tslib_1.__awaiter(this, void 0, void 0, function* () {
            const obj = request.body;
            (0, plugin_1.deletePluginProduction)(obj.pl_id, obj.pl_owner, obj.pl_name, obj.pl_description).then((result) => {
                if (result.ok) {
                    response.redirect('/productions/plugins');
                }
                else {
